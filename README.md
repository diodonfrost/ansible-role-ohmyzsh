...
# Pull updates on every run when the Oh My Zsh repository is already cloned.
# Disabled by default to preserve existing behavior; set to true to keep
# Oh My Zsh up to date.
ohmyzsh_update: false
...
* [Libvirt](https://libvirt.org/) (bsd test only)
* [Vagrant](https://www.vagrantup.com/downloads.html) (bsd test only)

### Testing with Docker

```shell
# Test ansible role with debian-12
image=debian:12 molecule test

# Test ansible role with ubuntu-24.04
image=ubuntu:24.04 molecule test

# Test ansible role with alpine-latest
image=alpine:latest molecule test

# Create debian-12 instance
image=debian:12 molecule create

# Apply role on debian-12 instance
image=debian:12 molecule converge

# Launch tests on debian-12 instance
image=debian:12 molecule verify
```
